...
function renderKitchenOrders(ordini) {
    ...
    Object.entries(sections).forEach(([stato, lista]) => {
        ...
        if (lista.length === 0) {
            grid.innerHTML = `<p class="no-orders" style="color:#64748b; font-style:italic; padding:10px;">Nessun ordine attivo</p>`;
        }
        ...
        lista.forEach(o => {
            ...
            card.innerHTML = `
                <div class="card-header" style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid #334155; padding-bottom:8px; margin-bottom:8px;">
                    <h3 style="margin:0; color:#38bdf8;">#${o.id.toString().slice(-4)}</h3>
                    <span class="badge" style="background:#475569; padding:4px 8px; border-radius:4px; font-size:0.75rem; font-weight:bold;">${o.tipo_ordine.toUpperCase()}</span>
                </div>
                ...
                ${o.indirizzo ? `<p style="margin:4px 0; font-size:0.9rem;"><strong>📍 Consegna:</strong> ${escapeHtmlInfallibile(o.indirizzo)}</p>` : ''}
